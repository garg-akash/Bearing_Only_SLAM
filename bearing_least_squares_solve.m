...
#   chi_stats_{l,r}: array 1:num_iterations, containing evolution of chi2 for landmarks and poses
#   num_inliers_{l,r}: array 1:num_iterations, containing evolution of inliers landmarks and poses

function [XR, XL, chi_stats_l, num_inliers_l, chi_stats_r, num_inliers_r, H, b] = doTotalLS(XR,XL,Zl,landmark_associations,Zr,pose_associations,num_iterations,damping,kernel_threshold)
  global pose_dim;
  global landmark_dim;
  ...
  for i=1:num_iterations
    ...
    if (num_landmarks)
      [H_landmarks, b_landmarks, chi_tot_l, num_inliers_]=buildLinearSystemLandmarks(XR,XL,Zl,landmark_associations,kernel_threshold);
    
      chi_stats_l(:,i)=chi_tot_l;
      num_inliers_l(:,i)=num_inliers_;
    endif;
  
    [H_poses, b_poses, chi_tot_r, num_inliers_]=buildLinearSystemPoses(XR,XL,Zr,pose_associations,kernel_threshold);
    
    chi_stats_r(:,i)=chi_tot_r;
    num_inliers_r(:,i)=num_inliers_;
