...
for i=1:length(landmark_unique_count)
  ...
  if (landmark_unique_count(i)==1)  
    ...
    R = S*(sin(landmark_unique_measure(1,row))/sin(delta_measure));
    r_vec(3,col) = r_vec(3,col) + landmark_unique_measure(1,col);
    r_vec(3,col) = atan2(sin(r_vec(3,col)),cos(r_vec(3,col)));
    XL_guess(:,i) = [r_vec(1,col)+R*cos(r_vec(3,col));r_vec(2,col)+R*sin(r_vec(3,col))];
  endif;
endfor;
#Triangulation ends
